...
**Why 1-to-2, not 1-to-3 (2026-05-26 update):** the previous 1-to-3 cap allowed positions like `K + Q vs K + 3 non-queen pieces` (e.g., K+Q vs K+2R+N) to count as balanced. Closer analysis showed all such positions are forceable for the +material side under optimal play. The two 0-bishop compositions in the 3-non-queen space are K+Q vs K+2R+N and K+Q vs K+2N+R; both are forceable. The hardest of the two is K+2R+N: rook geometry leaves exactly 1 square uncovered (e.g., Kf2, Rb7, Rc2, Nf6 covers all squares except h2; when K moves to g2 to threaten h2, e2 becomes uncovered instead), forcing B's Q-as-B into a 4-state oscillation between h2 and e2, terminating via the repetition rule. K+2N+R is easier because the knights' chebyshev-1 reactive jump-capture range plus chebyshev-2 movement cover 24 squares per knight, and with the rook + king covers all 60 non-W squares, leaving B's Q-as-B with zero safe teleport destinations (the bishop is captured directly rather than via repetition). With 1 or 2 bishops in the 3-non-queen mix, the bishops continuously pin B's queen, making the trap easier still. The 1-to-2 cap removes over-coverage of these positions; they win on material rather than via rule-driven termination.
