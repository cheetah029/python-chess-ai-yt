...
K+2N+R is the easier of the two. An example coverage layout is Nc3, Nf6, Rb7, Kg2. Knight coverage extends beyond the 16-square radius-2 movement range to include the 8 chebyshev-1 squares around the knight (its reactive jump-capture range), since bishop teleport-safety treats knight-jump-capturable squares as captureable per the rulebook's Bishop section. So each knight covers 24 squares (the chebyshev-≤2 box around it). Combined with R and K, this layout covers all 60 non-W squares. **In a position where a bishop (real or queen-in-bishop-form) has zero teleport-safe destinations, it has NO legal spatial moves at all** — bishop teleport is its only spatial mechanism, and the teleport-safety check rules out every destination. The bishop is not captured immediately by the coverage alone (it sits on a covered square but did not move, so no reactive jump-capture fires); rather, the bishop is denied movement, and the player's only legal turns with that piece are queen actions (transformations) for a queen-in-bishop-form, or NONE at all for a real bishop (real bishops have no actions, only the teleport move). Q-as-B then transforms out of bishop form to attempt escape, but each transformation costs a turn while W tightens the trap — capture follows from the resulting positional collapse or from the repetition rule, not from the initial coverage move.
