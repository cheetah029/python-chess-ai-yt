...
- **Remaining Goal 1 work = part (2):** the deliberate ruleset-finalization sweep. Be CONSERVATIVE — the ruleset is nearly final; genuine clarifications should be RARE. See the checklist below.
...
Resolved recently: repetition-rule invuln cycle (c7e0ffd), tiny endgame redesign (1c7cdec), bishop double-manip reactive capture (9d60689), boulder capture-return (60a2e4d). **(a) Goal 1's >6 stall question — RESOLVED 2026-05-20: ≤6 scope accepted as sufficient (see Goal 1 STATUS above).** Still to confirm before training: (b) a deliberate, CONSERVATIVE sweep for any remaining minor rule ambiguities (the kind that keep surfacing — manipulation/reactive-capture timing, self-capture semantics, boulder edge cases, knight invuln edge cases). Genuine clarifications should be RARE — do not over-identify. Treat RULEBOOK_v2.md as authoritative; keep docs/key-rule-differences.md + docs/potential-rule-changes.md in sync.
