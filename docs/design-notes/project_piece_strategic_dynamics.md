...
# Forks, pinning knights/rooks, and trade-favorability (user clarification, 2026-05-20)

- **Non-bishop pieces can't teleport, so they are inherently easier to catch** than bishops / queens-as-bishop. BUT **knights and rooks are still very mobile** and almost always need to be **pinned down by a bishop** to be constrained — otherwise they have too many available squares to flee to. So constraining a knight/rook usually requires a bishop pin (the reactive-capture threat) to cut its escape options.
- **Forks are the easiest way to force a capture or trade** (one piece attacking two enemy pieces so the opponent can't save both).
- **Critical constraint on any forced trade/capture:** it must be **FAVORABLE for the initiator in the resulting position** — which is NOT always the case. Under optimal play a side will not initiate (or walk into) a materially-equal trade if the resulting position is worse for it. So "a fork/capture is available" does NOT imply "a favorable trade is forced." Always check that the resulting position favors the initiator before treating a capture as forcing.
