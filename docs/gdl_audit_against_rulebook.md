...
| Loss: no legal turn available at start of turn | step 7 (2026-07-20, issue #162): `(<= (lost ?player) (true (control ?player)) (not (has_any_legal_turn ?player)))` — the unconditional rulebook rule, readable directly from the GDL | ✅ |
| Loss: repetition 3rd time | step 10 sketch (pure GDL) + HOST enforcement in `GGPGame` (issue #160, 2026-07-20) | ✅ RESOLVED HOST-SIDE: state hashing remains inexpressible in pure GDL-I (do not attempt), but the host wrapper now implements the rule — `GGPGame` counts each resulting state in `step()` (key = fact set minus `turn_number` / `state_repetition_count` / `distance_count`), filters third-occurrence moves in `legal_moves()` when `enforce_repetition=True` (a next-state query per move; leave off for search rollouts), and exposes the loss condition via `all_moves_repetition_blocked()`. The key is deliberately FINER than the engine's conditional hash (raw last-move fluents included) — the safe direction: the host never wrongly blocks a move the engine allows, it may detect a cycle one lap later. Direct `game.state` injection requires `reset_repetition_history()` |
...
| R1: manipulated piece can't make spatial move next own turn | step 7: `manipulation_freeze` set by manipulate / manipulate_promote; EVERY spatial-move legal rule now carries `(not (true (manipulation_freeze ?origin)))` (2026-07-20, issue #162 — the fluent was previously set but never consulted, so frozen pieces could still move; the audit's earlier claim that guards existed was wrong). The one-transition fluent lifetime exactly matches the engine's clear-at-manipulator's-next-turn timing; actions (transform) remain available to frozen pieces per the rulebook | ✅ |
...
| Reactive capture: enemy left bishop's diagonal LoS → bishop may capture next turn | step 9: `reactive_armed` FLUENT recorded by `next` rules at move time (2026-07-20 redesign, issue #154) + `reactive_capture` legal rule | ✅ Begin-time LoS (evaluated in the pre-move state, matching engine PR #151 — a mover landing on its own trail no longer disarms); the origin↔destination pairing is recorded atomically per move, resolving the old pairing caveat; `spatial_move_origin` removed. `jump_capture`/`reactive_capture` now also record `spatial_move_last_turn`; boulder moves record nothing |
