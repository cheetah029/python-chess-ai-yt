...
| Jump capture | n/a | **Proactive**: any enemy piece sitting on a square the knight can jump over may be jump-captured (provided the knight's landing square is empty). No timing condition. Only the jumped piece is captured — NOT adjacent pieces to landing. |
...
- ❌ **Knight jump-capture requires the jumped piece to have just moved.** No — that was the prior reactive rule. v2 jump-capture is now **proactive**: any enemy on the jumped square is capturable regardless of timing, as long as the landing square is empty. The reactive rule was removed because the hybrid (reactive jump + proactive standard capture) created a counterintuitive inversion (sitting enemy at chebyshev-1 safer than at chebyshev-2).
- ⚠️ **Double-manipulation jump-capture** is no longer a special case: under proactive jump-capture there is no timing condition for either side to consider. The "manipulated movement counts as moved" reasoning that mattered under reactive is moot now.
