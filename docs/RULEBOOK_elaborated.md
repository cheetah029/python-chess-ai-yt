...
This is **Version 2** of the rulebook. It differs from Version 1 (`RULEBOOK_v0.md`) in:
...
- The **Knight** has been redesigned. The previous always-on adjacent-capture rule (capture any enemy adjacent to the landing square after a jump) is replaced by two simpler mechanics:
  ...
  - **Invulnerability after jumping ("leap between friend and foe", remade 2026-06):** when the knight makes a **non-capturing** spatial move that jumps over a piece AND lands adjacent (chebyshev distance 1) to a piece of the **opposite allegiance** to the one it jumped, the knight is invulnerable to capture for the immediately following opponent turn. Jump a friendly/boulder → land beside an enemy; jump an enemy → land beside a friendly/boulder. The boulder is friendly-side in both roles and never an enemy. Captures of any kind — standard or accepted jump-capture — do not grant invulnerability; a *declined* jump-capture is a non-capturing enemy-vault and can grant it. The opposite-allegiance pairing keeps both design constraints: an enemy must always be involved (no own-camp stalling) and an enemy-vault needs friendly support at the landing (no lone infiltration).
...
The original RULEBOOK_v0.md is preserved as Version 1 for reference.
...
### **Invulnerability After Jumping ("leap between friend and foe")**

When the knight makes a **non-capturing** spatial move that jumps over a piece AND lands adjacent (chebyshev distance 1) to a piece of the **opposite allegiance** to the one it jumped, the knight is **invulnerable to capture during the immediately following opponent turn**:

* **Jump a friendly piece or the boulder → land adjacent to at least one enemy.** (This is the classic supported cavalry charge, unchanged from the previous version of the rule.)
* **Jump an enemy → land adjacent to at least one friendly piece or the boulder.** (New with the 2026-06 remake: the knight may crash *through* the enemy line, provided it lands where an ally backs it up.)

While invulnerable, no other piece may capture the knight via any move or action — this includes the king (friendly or enemy), whose special capture power does NOT override invulnerability. Invulnerability expires automatically when that opponent turn ends.

**The boulder counts as friendly-side in both roles** — as the jumped obstacle, and as the landing-adjacent support — and never counts as an enemy. This follows the general "the boulder is treated as a friendly piece by both sides" principle with no new exceptions.

**Why the opposite-allegiance form needs no exclusion clause.** The jumped square is always adjacent to the landing square (this is true for all three knight move families). Under formulations that measured both adjacencies at the landing, the jumped piece would trivially satisfy one of them, requiring careful "not counting the jumped piece" clauses. The opposite-allegiance form dissolves the problem: the piece you jumped and the piece you must land beside have opposite allegiances, so they can never be the same piece. There is nothing to exclude and nothing to remember.

**Both original design constraints remain enforced:**

1. *The enemy-involvement constraint* (anti-stall): invulnerability always requires an enemy in the pattern — either as the jumped piece or as the landing-adjacent piece. A knight bouncing over its own pieces in its own camp involves no enemy and earns nothing. This is what keeps the knight-versus-bishop endgame sound: a bishop pinning from chebyshev distance ≥ 3 on a clear diagonal can never be jumped (the only landing adjacent to it requires jumping a square *on* the pinning diagonal, which would have to be occupied — contradicting the pin) — so the knight cannot involve the bishop in any invulnerability pattern, and the reactive capture lands the moment the knight moves on the diagonal.

2. *The friendly-support constraint* (anti-infiltration): an enemy-vault grants protection only where a friendly piece or the boulder stands beside the landing. A lone knight deep in enemy territory has no such support — chaining leaps over enemy pieces earns nothing. The "catapult-over-the-attacker" abuse is likewise blocked: when an enemy steps adjacent to a supported knight and the knight vaults it, the knight's home-side supporter is left two squares behind — not adjacent to the landing — so no invulnerability is granted. Only a *forward* supporter (already adjacent to the landing square) enables the protected enemy-vault, which is precisely the "supported incremental advance" the rule intends to reward.

The adjacent piece may itself be **invulnerable**. The adjacency check is about *presence* — an opposing or supporting piece occupying the square — not about whether that piece can currently be captured.
...
**Declined jump-captures and the enemy-vault.** A knight may leap over an enemy *without capturing it*: either the enemy is not jump-capture-eligible (it did not move on the immediately preceding turn), or the offered jump-capture is declined. Both are non-capturing leaps over an enemy, and both **can grant invulnerability** if a friendly piece or the boulder is adjacent to the landing. (Under the previous rule, declining a jump-capture never granted invulnerability; the remake makes the decline a genuine tactical option — leave the enemy standing, take the armor instead.)
...
The rule models the knight **leaping between friend and foe**. In the classic direction, a cavalry charge launches from friendly lines: the knight vaults one of its own troops (or the boulder) into close quarters with the enemy. In the remade mirror direction, the knight crashes *through* the enemy line into a position its own side already holds — bursting past a foe to rejoin its comrades. Both are committed, supported maneuvers, and both briefly make the rider hard to strike. What earns no protection is the unsupported move: leaping in empty space (no engagement) or weaving alone through enemy territory (no support). One side of the leap must be friend, the other foe — the knight is armored precisely when it rides the boundary between them.

#### Design history (non-normative)

The previous version required the jumped piece to be friendly/boulder and the landing to be adjacent to a non-jumped enemy — enemy-vaults never granted invulnerability. The 2026-06 remake generalized it to the opposite-allegiance form after exploring and rejecting: an origin-adjacency "catapult" variant (abusable — an attacker stepping adjacent to a supported knight handed it a protected escape vault), and a both-adjacencies-at-landing variant (required awkward jumped-piece exclusion clauses). The full ruleset as of immediately before the remake is preserved in `snapshots/rules_v2.0_pre_knight_invuln_remake/` and the git tag `rules-v2.0-pre-knight-invuln-remake`; the model `models/variant_freeze_v3/model_iter_0500.pt` was trained on the pre-remake rule.
