...
**Manipulation as a positional-disruption tool (NOT a reactive-capture trigger).** A queen in base form can manipulate enemy pieces (with restrictions), relocating the target and freezing it for one turn (Restriction 1). **Correction (2026-05-19):** manipulation does NOT chain into a friendly bishop's reactive capture. Reactive capture only fires on the bishop's IMMEDIATE next turn after the trigger move; a manipulation by the bishop's own side is on that side's turn, so an opponent turn intervenes before the bishop can act and the window expires. The only timing-valid manipulation→bishop case is self-capture (opponent manipulates the bishop-owner's own piece off the LOS). So manipulation is a disruption/tempo tool, not an offensive capture-forcing tool against enemy bishops.
